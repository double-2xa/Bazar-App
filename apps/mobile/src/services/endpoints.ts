...
export const authApi = {
  ...
  registerCompany: (data: Record<string, string>) =>
    api
      .post<CompanyRegisterResponse>("/auth/register-company", data)
      .then((r) => r.data),
...
export const cartApi = {
  ...
  addItem: (productId: string, quantity: number, selectedPriceType?: string) =>
    api
      .post("/cart/items", { productId, quantity, selectedPriceType })
      .then((r) => r.data),
...
export const locationsApi = {
  getHierarchy: () =>
    api
      .get<
        import("@doublea/shared").LebanonHierarchy
      >("/locations/lebanon/hierarchy")
      .then((r) => r.data),
  getSettlements: (params?: {
    ...
    api
      .get<{
        total: number;
        data: import("@doublea/shared").LebanonSettlement[];
      }>("/locations/lebanon/settlements", { params })
      .then((r) => r.data),
};
  ...
  getMyOrders: (page = 1, limit = 20) =>
    api
      .get<{
        ...
        limit: number;
        totalPages: number;
      }>("/orders/my-orders", { params: { page, limit } })
      .then((r) => r.data),
...
export const notificationsApi = {
  ...
  getAll: (unreadOnly = false) =>
    api
      .get<AppNotification[]>("/notifications", { params: { unreadOnly } })
      .then((r) => r.data),
  markRead: (id: string) =>
...
export const deliveryApi = {
  ...
  getCompletedOrders: (page = 1, limit = 20) =>
    api
      .get<{
        ...
        limit: number;
        totalPages: number;
      }>("/delivery/orders/completed/history", { params: { page, limit } })
      .then((r) => r.data),
  ...
  markDelivered: (id: string, data?: DeliveryProofInput) =>
    api
      .patch<Order>(`/delivery/orders/${id}/delivered`, data ?? {})
      .then((r) => r.data),
};
