...
export default function OrderDetailPage() {
  ...
  const loadOrder = useCallback(() => {
    ...
    adminOrdersApi
      ...
      .then((data) => {
        setOrder(data);
        setSelectedAgentId(
          data.deliveryAgent?.id ?? data.deliveryAgentId ?? "",
        );
      })
      .catch((err) =>
  ...
  const markPackingFinished = async () => {
    ...
    try {
      ...
      setOrder(updated);
      setSelectedAgentId(
        updated.deliveryAgent?.id ?? updated.deliveryAgentId ?? "",
      );
      flashSuccess(
        "Order prepared / packed. Step 3 is unlocked — assign a driver.",
      );
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <span
            className={`badge ${delivery.badge}`}
      ...
      <ol className="order-flow-steps">
        <li
          className={`order-flow-step ${step1Done ? "is-done" : ""} is-current`}
        >
        ...
        <li
          className={`order-flow-step ${step2Done ? "is-done" : ""} ${preparing ? "is-current" : ""}`}
        >
        ...
        <li
          className={`order-flow-step ${step3Active ? "is-done is-current" : "is-locked"}`}
        >
          <span className="order-flow-step__num">3</span>
          <span>
            <strong>Assign driver</strong>
            <small>
              {step3Active ? "Delivery management" : "Unlocks after packing"}
            </small>
          </span>
        </li>
      </ol>
      ...
      <section
        className="card order-information-card"
        ...
        <div className="order-information-grid">
            ...
            {order.address ? (
              <>
                ...
                {order.address.hasExactLocation ? (
                  <p style={{ color: "#2E9D58", fontSize: 13 }}>
                    Exact GPS pin stored securely
                  </p>
                ) : null}
          ...
          <article className="order-info-location">
            <div className="order-info-location__heading">
              ...
              {hasExactDeliveryLocation ? (
                <span className="badge badge-success">Secure GPS</span>
              ) : null}
            </div>
            {hasExactDeliveryLocation ? (
              ...
              <div className="order-location-empty">
                <span className="order-location-empty__pin" aria-hidden>
                  ⌖
                </span>
                <strong>No exact location to display</strong>
                <p>
                  The written delivery address remains available for the driver.
                </p>
              </div>
            )}
          </article>
      ...
      <section className="card" style={{ marginBottom: 24 }}>
        ...
        {preparing ? (
          <div className="order-pack-action">
            <div>
              <p className="order-pack-action__title">
                Done packing this order?
              </p>
              <p>
                Click below when all items are prepared. That unlocks Step 3 so
        ...
        {preparing ? (
          <div className="delivery-locked-body">
            <p className="delivery-locked-msg">
              This step stays grayed out until you click{" "}
              <strong>Order prepared / packed</strong> in Step 2.
            </p>
            <div className="delivery-locked-preview" aria-hidden="true">
              <select
                disabled
                style={{ minWidth: 260, padding: "10px 14px", borderRadius: 8 }}
              >
                    ...
                    >
                      ...
                      {activeAgents.map((agent) => (
                        <option key={agent.id} value={agent.id}>
                          {agent.fullName} — {agent.activeOrderCount} active
                          order
        ...
        <div className="card order-status-history-card">
          <div className="order-status-history__header">
            ...
            {(order.statusHistory?.length ?? 0) > 3 ? (
              <span className="order-status-history__hint">
                Latest first · scroll for older
              </span>
            ) : null}
          </div>
              ...
              {[...order.statusHistory]
                ...
                .map((entry, index) => (
                  <div
                    ...
                    className="order-status-history__entry"
                  >
