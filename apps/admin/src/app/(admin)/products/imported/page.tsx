...
export default function ImportedProductsPage() {
  ...
  const removeSelected = async () => {
    if (!selected.length || !window.confirm(`Delete ${selected.length} selected imported draft${selected.length === 1 ? '' : 's'}? This cannot be undone.`)) return;
    setDeleting(true); setError('');
    try {
      const response = await api.delete<{ deleted: number }>('/product-imports/rows', { data: { rowIds: selected } });
      setMessage(`${response.data.deleted} imported draft${response.data.deleted === 1 ? '' : 's'} deleted.`);
      await load();
    } catch (err) { setError(getApiErrorMessage(err, 'Could not delete the selected drafts.')); }
    finally { setDeleting(false); }
  };

  const selectableRows = rows.filter((row) => row.status !== 'published');
