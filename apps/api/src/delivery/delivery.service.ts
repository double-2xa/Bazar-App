...
export class DeliveryService {
  ...
  async lockOrder(agentId: string, orderId: string) {
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (updated.count === 0) {
        ...
        throw new ConflictException('Order was already locked by another driver');
      }
  ...
  async markDelivered(agentId: string, orderId: string, dto: DeliveryProofDto) {
    ...
    if (!dto.agentSignatureDataUrl?.trim() || !dto.clientSignatureDataUrl?.trim()) {
      throw new BadRequestException('Both driver and client signatures are required');
    }
      ...
      await tx.order.update({
        ...
        data: {
          ...
          statusHistory: {
            create: {
              status: 'delivered',
              note: dto.deliveryNote || 'Delivered successfully with signatures',
              changedByUserId: agentId,
            },
          },
        },
